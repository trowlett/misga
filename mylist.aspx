...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" Runat="Server">
    <h2>
		...
		<asp:Button ID="btnSelect" runat="server" Text="Show Events" 
			...
			SelectCommand="SELECT ClubID, PlayerID, Name, LName, FName, Hcp, MemberID, Sex, Title, HDate, [Delete] FROM Players WHERE (ClubID = @ClubID) ORDER BY LName, FName" ProviderName="System.Data.SqlClient">
                    <SelectParameters>
                        <asp:Parameter DefaultValue="999" Name="ClubID" Type="String" />
                    </SelectParameters>
		</asp:SqlDataSource>
		...
		<table>
			...
			<tr id="Tr1" class='<%# ((PrEvent)(Container.DataItem)).PrType.ToLower().Trim() %>' runat="server" visible='<%# !((PrEvent)Container.DataItem).PrCompleted %>'>
				<td class="futurecol"><%# ((PrEvent)Container.DataItem).PrCompleted == true ? "Y" : "" %></td>
					...
					<td class="datecol"><a href='Signup.aspx?CLUB=<%# Eval("PrClubID") %>&ID=<%# Eval("PrEvID") %>'><%# ((PrEvent)Container.DataItem).PrDate.ToString("ddd, MMM d") %></td>
					<td class="center"><a href='Signup.aspx?CLUB=<%# Eval("PrClubID") %>&ID=<%# Eval("PrEvID") %>'><%# ((PrEvent)Container.DataItem).PrType %></td>
					<td class="clubcol"><a href='Signup.aspx?CLUB=<%# Eval("PrClubID") %>&ID=<%# Eval("PrEvID") %>'><%# ((PrEvent)Container.DataItem).PrTitle %></td>
					<td class="center"><a href='Signup.aspx?CLUB=<%# Eval("PrClubID") %>&ID=<%# Eval("PrEvID") %>'><%# ((PrEvent)Container.DataItem).PrCost %></td>
					<td class="center"><a href='Signup.aspx?CLUB=<%# Eval("PrClubID") %>&ID=<%# Eval("PrEvID") %>'><%# ((PrEvent)Container.DataItem).PrTime %></td>
					<td class="deadlinecol"><a href='Signup.aspx?CLUB=<%# Eval("PrClubID") %>&ID=<%# Eval("PrEvID") %>'><%# ((PrEvent)Container.DataItem).PrDeadline.ToString("MMM d") %></td>
					<td class="center"><a href='Signup.aspx?CLUB=<%# Eval("PrClubID") %>&ID=<%# Eval("PrEvID") %>'><%# ((PrEvent)Container.DataItem).PrCarpool %></td>
				</tr>
