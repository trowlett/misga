 ...
 * */
    ...
    private static string[] _misgadesc = {   "<!-- s/w generated -->\n\r<h2>",
                                            "1 - Title ",
                                            "</h2>\n\r<h3>Date:&nbsp;&nbsp;",
                                            "3 - date",
                                            "</h3>\n\r<div id=\"details\">\n\r<table><tr><td><h4>Player Limit:&nbsp;&nbsp;",
...
//		_FileName = path + "\\Pages\\" + EventID + ".txt";
    ...
    private static bool clubInDatabase(string eventID)
    {
        ...
        if (item != null)
        {
            ...
            if (mixer.Type.ToUpper().Trim() == "MISGA") _lines = fillMISGADescription(item, mixer);
    ...
    private static string IsSpecialRule(Events m, Clubs i)
    {
        ...
        if (m.SpecialRule.Trim() != "")
        {
            string tc = "";
            if (i.TChoice != null) tc = i.TChoice.Trim();
            rst = "<div id=\"rule\">\n\r<h5>Tee Choices</h5><p>" + i.SRule.Trim() + tc + "</p>\n\r</div>";
        }
        return rst;
    }
    ...
    private static string[] fillClubDescription(Clubs item, Events mixer)
    {
        ...
        _clubdesc[17] = IsSpecialRule(mixer, item);
        return _clubdesc;
    }
    private static string[] fillMISGADescription(Clubs item, Events mixer)
    {
        _misgadesc[1] = mixer.Title;
        _misgadesc[3] = mixer.Date.ToLongDateString();
        _misgadesc[5] = IsUnlimited(mixer.PlayerLimit);
        _misgadesc[7] = string.Format("{0:h:mm tt} - {1:D}", mixer.Deadline, mixer.Deadline);
        _misgadesc[9] = SetCost(mixer.Cost);
        _misgadesc[11] = item.PayOpt;
        _misgadesc[13] = mixer.Date.ToShortTimeString();
        _misgadesc[15] = item.Refresh;
        _misgadesc[17] = string.Format("Slope for {0} is {1}", item.ClubName, item.slope);
        _misgadesc[19] = item.MISGAURL;
        _misgadesc[21] = item.ProPhone;
        _misgadesc[23] = mixer.PlayerLimit < 50 ? _limitMsg : " ";
        _misgadesc[24] = PolicyAndMisc(item);
        _misgadesc[25] = IsSpecialRule(mixer, item);
        return _misgadesc;
    }
